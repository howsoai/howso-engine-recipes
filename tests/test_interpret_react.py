...
def test_predicted_mean_interval(tb):
    ...
    tb.inject(
        """
        global_residual = trainee.react_aggregate(details={"prediction_stats":True}).to_dataframe()['moid']['mae']
...
def test_boundary_values(tb):
    ...
    assert 0.16 <= boundary_values['e'][1] <= 0.38

    assert 1.50 <= boundary_values['q'][0] <= 2.00
